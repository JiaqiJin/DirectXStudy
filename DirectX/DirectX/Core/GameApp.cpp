...
bool GameApp::InitResource()
{
	...
	vbd.Usage = D3D11_USAGE_IMMUTABLE; //type specifying how the buffer will be used.
	vbd.ByteWidth = sizeof vertices;
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbd.CPUAccessFlags = 0; //Specifies how the CPU will access the buffer.
	// 新建顶点缓冲区 // Specify the data to initialize the index buffer.
	D3D11_SUBRESOURCE_DATA InitData;
	ZeroMemory(&InitData, sizeof(InitData));
	InitData.pSysMem = vertices;//pointer to a system memory array which contains the data to initialize the vertex buffer
	...
	//bind it to the pipeline 输入装配阶段的索引缓冲区设置
	...
	cbd.Usage = D3D11_USAGE_DYNAMIC; //permitir que el búfer constante se escriba desde la CPU.
	...
	// Initialize the value of the constant buffer 初始化常量缓冲区的值
	m_CBuffer.world = XMMatrixIdentity();	// 单位矩阵的转置是它本身 The transpose of the identity matrix is itself
